...
export function WeaknessListSection({ refreshSignal = 0 }: Props) {
  ...
  const renderRow = (item: UserWeaknessItem) => (
    <Swipeable
      key={item.id}
    ...
    >
      <View style={[styles.rowCard, { backgroundColor: cardBg, borderColor: borderSoft }]}>
        ...
        >
          <Text style={[styles.rowTitle, { color: text }]}>{weaknessListPreviewTitle(item)}</Text>
          <Text style={[styles.rowSub, { color: outline }]}>{weaknessListPreviewDetail(item)}</Text>
  ...
  return (
    <>
      <Pressable
        ...
        style={({ pressed }) => [
          styles.addBtn,
          { borderColor: `${weaknessAccent}55`, opacity: pressed ? 0.85 : 1 },
        ]}
      >
        <MaterialIcons name="add-circle-outline" size={20} color={weaknessAccent} />
        <Text style={[styles.addBtnText, { color: weaknessAccent }]}>添加缺点</Text>
      </Pressable>
      ...
      {loading ? (
        ...
        <View style={[styles.emptyCard, { backgroundColor: cardBg, borderColor: borderSoft }]}>
          <MaterialIcons name="psychology-alt" size={36} color={outline} />
...
const styles = StyleSheet.create({
  ...
  emptyHint: { fontSize: 14, textAlign: 'center', lineHeight: 21, fontWeight: '600' },
  listBlock: { gap: 12 },
  swipeListHint: { fontSize: 11, fontWeight: '600', opacity: 0.9, paddingHorizontal: 2, marginBottom: 2 },
  rowWrap: {},
  swipeDeleteAction: {
    width: 88,
    justifyContent: 'center',
