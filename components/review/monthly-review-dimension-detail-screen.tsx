...
const PAGE_API_KEY = 'monthly-review-dimension-detail';
...
export function MonthlyReviewDimensionDetailScreen() {
  ...
  const dimensionId =
    (Array.isArray(params.dimensionId) ? params.dimensionId[0] : params.dimensionId)?.trim() ?? '';
  ...
  const dimension = useMemo(
    () => monthlyTemplate.find(d => d.id === dimensionId) ?? null,
    [monthlyTemplate, dimensionId],
  );
  ...
  const reload = useCallback(async () => {
    if (!monthStartYmd || !dimensionId) return;
    ...
    try {
      await wrapLoad(async () => {
        ...
        setCanEdit(isMonthlyReviewEditable(monthStartYmd, todayYmd));
        setMonthLabel(formatReviewMonthLabel(monthStartYmd));
        ...
        const dim = tpl.find(d => d.id === dimensionId);
        const dimColIds = dim?.columns.map(c => c.id) ?? [];
        setDimensionTitle(dim?.title ?? '月复盘详情');
  ...
  }, [dimensionId, monthStartYmd, todayYmd, wrapLoad]);
  ...
  const persist = useCallback(async (opts?: { force?: boolean }) => {
    if (!canEdit || !monthStartYmd) return;
    ...
    try {
      ...
      console.warn('monthly review dimension save', e);
    } finally {
      setSaving(false);
    }
  ...
  const runAi = useCallback(async () => {
    ...
    try {
      ...
      const text = await generateReviewAiAnalysis({
        scope: 'monthly',
        periodLabel: monthLabel || formatReviewMonthLabel(monthStartYmd),
  ...
  const applyToolbarAction = useCallback(
    async (kind: 'font' | 'todo' | 'image' | 'time' | 'undo' | 'redo') => {
      ...
      if (kind === 'redo') {
        ...
        undoStacksRef.current[columnId] = [
          ...(undoStacksRef.current[columnId] ?? []),
          currentSerialized,
        ].slice(-MAX_HISTORY);
      ...
      if (!block || block.kind !== 'text') {
        if (kind === 'image') {
          ...
          if (!asset?.uri) return;
          const inserted = insertImageBlock(
            currentModel,
            currentModel.blocks.length - 1,
            { start: 0, end: 0 },
            asset.uri,
          );
      ...
      if (kind === 'font') {
        ...
        const nextBlocks = currentModel.blocks.map((b, idx) =>
          idx === editorState.blockIndex && b.kind === 'text'
            ? { kind: 'text' as const, model: result.model }
            : b,
        );
  ...
  if (!monthStartYmd || !dimensionId) {
    return (
      <SafeAreaView style={[styles.safe, { backgroundColor: colors.background }]}>
        <ScreenHeader title="月复盘详情" onBack={() => router.back()} />
        ...
        right={
          <Pressable
            ...
            {saving ? (
              ...
              <Text style={[Typography.bodyStrong, { color: canEdit ? colors.primary : colors.textMuted }]}>
                保存
              </Text>
            )}
          </Pressable>
