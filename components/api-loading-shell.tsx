...
/**
 * 统一全局 API 加载壳：内容区淡入淡出 + 顶部进度条 / 阻塞蒙层 / 超时重试。
 * 骨架屏 Tab 仅顶部进度条，不遮挡页面内骨架。
 */
export function ApiLoadingShell({ children, style }: ApiLoadingShellProps) {
  ...
  return (
    <View style={styles.root}>
      <Animated.View style={[styles.content, style, { opacity }]}>{children}</Animated.View>
      <ApiLoadingOverlay />
    </View>
  );
}

/** @deprecated 使用 ApiLoadingShell；保留别名兼容旧 import */
export function ApiContentTransition({ children, style }: ApiLoadingShellProps) {
  return <ApiLoadingShell style={style}>{children}</ApiLoadingShell>;
}

/** @deprecated 已并入 ApiLoadingShell；单独挂载时仅渲染蒙层 */
